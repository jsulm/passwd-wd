...
exports.saveErrorDetailsInRequest = function(req, errorDetails) {
  req.readyToLogError = {};
  req.readyToLogError.code = errorDetails.code;
  req.readyToLogError.text = errorDetails.text;
  req.readyToLogError.result = req.locals.ip + '/' + req.locals.port;
};
...
exports.processErrorDetails = function(req, res, next) {
  if (req.readyToLogError !== undefined) {
    logger.log('warn', req.readyToLogError);
    logger.log('warn', 'Sending to client: ' + req.readyToSend.code);
  }
...
exports.sendResults = function(req, res) {
  ...
  // Info logging
  let ip = (req.locals !== undefined) ? req.locals.ip : '';
